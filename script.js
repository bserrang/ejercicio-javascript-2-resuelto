...
function removeElements() {
  ...
  elementsToRemove.forEach(el => firstList.removeChild(el));
}
...
function generateList() {
  ...
  firstListChilds.forEach(el => {
    const elChild = el.firstChild;
    const item = document.createElement("li");
    const button = document.createElement("button");
    const text = document.createTextNode(elChild.textContent);
    button.className = elChild.className;
